// ✅✅✅ 전체복붙: src/app/admin/users/page.tsx
'use client';
...
function safeImgSrc(avatar_url?: string | null) {
  const raw = getAvatarSrc(avatar_url ?? null);
...
function isMissingRelationOr404(msg: string) {
...
export default function AdminUsersPage() {
  ...
  const [sanctionsAll, setSanctionsAll] = useState<SanctionRow[]>([]);

  const [badgesDisabled, setBadgesDisabled] = useState(false);
  const [sanctionsDisabled, setSanctionsDisabled] = useState(false);
  ...
  async function fetchMonthlyBadgesWithReason(userId: string) {
    ...
    if (basic.error) {
      const msg = basic.error.message || report.error.message;
      setBadgeErr(msg);
      setBadges([]);
      setBadgeSource('none');
      if (isMissingRelationOr404(msg)) setBadgesDisabled(true);
    } else {
      const withPlaceholder = (((basic.data as any[]) ?? []) as any[]).map((b) => ({ ...b, reason: null }));
  ...
  async function fetchSanctionsAll(userId: string) {
    if (sanctionsDisabled) return;
    ...
    if (res.error) {
      ...
      setSanctionsAll([]);
      if (isMissingRelationOr404(res.error.message)) setSanctionsDisabled(true);
      setSanLoading(false);
      return;
    }

    setSanctionsAll(((res.data as any[]) ?? []) as SanctionRow[]);
    setSanLoading(false);
  }
  ...
  function openPanel(p: ProfileRow) {
    ...
    if (lastPanelUserIdRef.current === p.user_id) return;
    lastPanelUserIdRef.current = p.user_id;

    if (!badgesDisabled) fetchMonthlyBadgesWithReason(p.user_id);
    if (!sanctionsDisabled) fetchSanctionsAll(p.user_id);
  }

  async function grantSanction() {
    if (!selected) return;
    if (sanctionsDisabled) {
      setGrantMsg('⚠ sanctions 테이블/권한이 없어 제재 부여를 할 수 없습니다.');
      return;
    }
    ...
    setGrantBusy(false);
    fetchSanctionsAll(selected.user_id);
  }

  async function revokeSanction(id: string) {
    if (!selected) return;
    if (sanctionsDisabled) {
      alert('sanctions 테이블/권한이 없어 해제가 불가합니다.');
      return;
    }
  ...
  const activeSanctions = useMemo(() => sanctionsAll.filter(isActiveSanction), [sanctionsAll]);
  const historySanctions = useMemo(() => sanctionsAll.filter((s) => !isActiveSanction(s)), [sanctionsAll]);
    ...
    <div className="page">
      ...
      <div className="top">
        ...
        <div className="navBtns">
          <button type="button" className="navBtn" onClick={() => router.push('/admin')}>
            ← 관리자 대시보드
          </button>
          <button type="button" className="navBtn strong" onClick={() => fetchProfiles()} disabled={loading}>
            새로고침
          </button>
        </div>
      </div>
      ...
      {(badgesDisabled || sanctionsDisabled) && (
        <div className="warn">
          {badgesDisabled ? '⚠ 월간 배지 리소스(뷰/테이블)가 없어 호출을 중단했습니다. ' : ''}
          {sanctionsDisabled ? '⚠ sanctions 테이블/권한 문제가 있어 제재 호출을 중단했습니다.' : ''}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="grid">
          {filtered.map((p) => {
            ...
            const avatarSrc = safeImgSrc(p.avatar_url);
            ...
            return (
              <div key={p.user_id} className="card">
                <div className="cardTop">
                  ...
                  <button type="button" className="viewBtn" onClick={() => openPanel(p)}>
                    보기
                  </button>
                </div>
      ...
      <aside className={`panel ${panelOpen ? 'on' : ''}`}>
        <div className="panelHead">
          ...
          <button type="button" className="panelClose" onClick={() => setPanelOpen(false)} aria-label="닫기">
            ✕
          </button>
        </div>
        ...
        {!selected ? (
          ...
          <div className="panelBody">
            {(() => {
              ...
              const avatarSrc = safeImgSrc(selected.avatar_url);
              ...
              return (
                <>
                  ...
                  <div className="panelSection">
                    ...
                    {badgesDisabled ? (
                      <div className="miniErr">
                        ⚠ 월간 배지 리소스가 없어 조회를 중단했습니다.
                        <div className="miniHint">(monthly_badges 또는 monthly_badge_winners_report 뷰/권한 확인 필요)</div>
                      </div>
                    ) : badgeLoading ? (
                      ...
                      <div className="badgeList">
                        {badges.map((b, idx) => (
                          <div key={`${b.badge_name ?? b.badge_code ?? 'badge'}-${idx}`} className="badgeItem">
                            <div className="badgeDot" />
                            <div className="badgeText">
                              <div className="badgeName">{b.badge_name ?? '배지'}</div>
                              {safeStr(b.reason) ? (
                                <div className="badgeReason">{safeStr(b.reason)}</div>
                              ) : (
                                <div className="badgeReason dim">사유 데이터 준비중</div>
                              )}
                            </div>
                          </div>
                        ))}
                  ...
                  {/* ✅ 제재 */}
                  <div className="panelSection">
                    <div className="secTitle">제재 관리</div>

                    {sanctionsDisabled ? (
                      <div className="miniErr">
                        ⚠ sanctions 테이블/권한 문제로 조회를 중단했습니다.
                        <div className="miniHint">(sanctions 테이블 생성 + RLS/관리자 권한 필요)</div>
                      </div>
                    ) : sanLoading ? (
                      <div className="miniMsg">불러오는 중…</div>
                    ) : sanErr ? (
                      ...
                      <>
                        <div className="splitTitle">
                          <span className="splitLabel on">현재 활성</span>
                          <span className="splitCount">{activeSanctions.length}건</span>
                        </div>
                          ...
                          <div className="sanList">
                            {activeSanctions.map((s) => (
                              <div key={s.id} className="sanItem">
                                ...
                                <button type="button" className="sanKill" onClick={() => revokeSanction(s.id)}>
                                  해제
                                </button>
                              </div>
                            ))}
                        ...
                        <div className="splitTitle">
                          <span className="splitLabel">과거/만료</span>
                          <span className="splitCount">{historySanctions.length}건</span>
                        </div>

                        {historySanctions.length === 0 ? (
                          <div className="miniMsg">과거 제재 내역이 없습니다.</div>
                        ) : (
                          <div className="sanList faded">
                            {historySanctions.slice(0, 6).map((s) => (
                              <div key={s.id} className="sanItem">
                                <div className={`sanPill ${s.is_hard ? 'hard' : 'soft'}`}>{s.is_hard ? '하드' : '소프트'}</div>
                                <div className="sanMain">
                                  <div className="sanLine">
                                    <b>{safeStr(s.scope) || 'scope'}</b> · {safeStr(s.action) || 'action'}
                                  </div>
                  ...
                  <div className="panelFoot">
                    <button type="button" className="footBtn" onClick={() => setPanelOpen(false)}>
                      닫기
                    </button>
                  </div>
                </>
...
.overlay{position:absolute;inset:0;pointer-events:none;background: radial-gradient(900px 500px at 60% 10%, rgba(255,79,216,0.10), transparent 60%);}

.top{position:relative;max-width:1120px;margin:0 auto;padding:28px 18px 12px;display:flex;align-items:flex-end;justify-content:space-between;gap:12px;}
...
.navBtn{border:1px solid rgba(18,6,26,0.16);background: rgba(255,255,255,0.82);padding:10px 14px;border-radius:999px;font-weight:950;color:#12061a;box-shadow: 0 10px 28px rgba(40,10,70,0.08);}
.navBtn.strong{background: linear-gradient(135deg, rgba(255,79,216,0.22), rgba(185,130,255,0.22));border-color: rgba(255,79,216,0.26);}
...
.warn{position:relative;max-width:1120px;margin:0 auto;padding:10px 18px;color:#5a2a7a;font-weight:1000;}
.loading,.empty{position:relative;max-width:1120px;margin:14px auto 0;padding:18px;font-weight:950;color:#12061a;}
.miniHint{margin-top:8px;font-size:12px;font-weight:950;color: rgba(11,11,15,0.60);}

.grid{position:relative;max-width:1120px;margin:0 auto;padding:10px 18px 70px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;}
@media (max-width:920px){.grid{grid-template-columns:1fr;}.top{align-items:flex-start;flex-direction:column;}}

.card{background: rgba(255,255,255,0.86);border: 1px solid rgba(18,6,26,0.12);border-radius: 20px;box-shadow: 0 18px 40px rgba(40,10,70,0.10);padding:14px 14px 12px;backdrop-filter: blur(10px);}
...
.miniAvatar{width:52px;height:52px;border-radius:18px;object-fit:cover;background:#fff;border:1px solid rgba(18,6,26,0.10);box-shadow:0 10px 22px rgba(40,10,70,0.10);flex:0 0 auto;}
...
.actBtn{flex:1;height:40px;border-radius:14px;border:1px solid rgba(18,6,26,0.14);font-weight:1000;color:#0b0b0f;background: rgba(255,255,255,0.90);box-shadow:0 12px 24px rgba(40,10,70,0.08);}
...
.panelClose{width:38px;height:38px;border-radius:12px;border:1px solid rgba(18,6,26,0.14);background: rgba(255,255,255,0.90);font-weight:1000;color:#0b0b0f;box-shadow:0 10px 22px rgba(40,10,70,0.10);}
.panelBody{padding:0 16px 16px;overflow:auto;}
.panelEmpty{padding:16px;font-weight:950;color:#0b0b0f;}

.heroCard{margin-top:6px;border-radius:18px;border:1px solid rgba(18,6,26,0.12);background: rgba(255,255,255,0.84);box-shadow:0 14px 30px rgba(40,10,70,0.10);padding:14px;display:flex;gap:12px;align-items:flex-start;}
.heroAvatar{width:72px;height:72px;border-radius:20px;object-fit:cover;background:#fff;border:1px solid rgba(18,6,26,0.10);box-shadow:0 14px 30px rgba(40,10,70,0.12);flex:0 0 auto;}
...
.suspendTitle{font-size:13px;font-weight:1000;color:#0b0b0f;}
.suspendDesc{margin-top:4px;font-size:12px;font-weight:950;color: rgba(11,11,15,0.68);}
.suspendBtns{display:flex;gap:8px;flex-wrap:wrap;}
.sBtn{height:38px;padding:0 12px;border-radius:14px;border:1px solid rgba(18,6,26,0.14);font-weight:1000;color:#0b0b0f;box-shadow:0 12px 22px rgba(40,10,70,0.08);background: rgba(255,255,255,0.92);white-space:nowrap;}
.sBtn.ban{background: linear-gradient(135deg, rgba(255,94,122,0.18), rgba(255,154,174,0.14));border-color: rgba(255,94,122,0.26);}
.sBtn.lift{background: linear-gradient(135deg, rgba(73,183,255,0.16), rgba(143,215,255,0.12));border-color: rgba(73,183,255,0.26);}

.panelSection{margin-top:12px;border-radius:18px;border:1px solid rgba(18,6,26,0.12);background: rgba(255,255,255,0.84);box-shadow:0 14px 30px rgba(40,10,70,0.10);padding:12px;}
...
.splitTitle{margin-top:10px;display:flex;align-items:center;justify-content:space-between;gap:10px;}
.splitLabel{display:inline-flex;align-items:center;gap:8px;font-size:12px;font-weight:1000;color: rgba(11,11,15,0.72);}
.splitLabel.on{color:#0b0b0f;}
.splitCount{font-size:12px;font-weight:1000;color: rgba(11,11,15,0.60);}
.divider{margin:12px 0;height:1px;background: rgba(18,6,26,0.10);}
...
.sanItem{display:flex;gap:10px;align-items:flex-start;padding:10px;border-radius:14px;border:1px solid rgba(18,6,26,0.10);background: rgba(255,255,255,0.78);}
.sanPill{padding:6px 10px;border-radius:999px;font-size:11px;font-weight:1000;border:1px solid rgba(18,6,26,0.12);white-space:nowrap;}
.sanPill.soft{background: rgba(255,79,216,0.12);border-color: rgba(255,79,216,0.22);color:#0b0b0f;}
.sanPill.hard{background: rgba(255,94,122,0.14);border-color: rgba(255,94,122,0.26);color:#0b0b0f;}
...
.expired{color: rgba(255,94,122,0.85);font-weight:1000;}
.sanNote{margin-top:6px;font-size:12px;font-weight:950;color: rgba(11,11,15,0.75);line-height:1.35;}
.sanKill{height:34px;padding:0 10px;border-radius:12px;border:1px solid rgba(18,6,26,0.12);background: rgba(255,255,255,0.92);font-weight:1000;color:#0b0b0f;box-shadow:0 10px 18px rgba(40,10,70,0.08);}
.ghostChip{height:34px;padding:0 10px;border-radius:12px;border:1px dashed rgba(18,6,26,0.16);background: rgba(18,6,26,0.04);font-weight:1000;color: rgba(11,11,15,0.65);display:flex;align-items:center;}
.refreshRow{margin-top:10px;display:flex;justify-content:flex-end;}

.grantHint{margin-top:6px;font-size:12px;font-weight:950;color: rgba(11,11,15,0.65);}
.grantRow{margin-top:10px;display:grid;grid-template-columns:56px 1fr;gap:10px;align-items:flex-start;}
.lbl{font-size:12px;font-weight:1000;color:#0b0b0f;padding-top:8px;}
.seg{display:flex;gap:8px;flex-wrap:wrap;}
.chip{height:34px;padding:0 12px;border-radius:999px;border:1px solid rgba(18,6,26,0.14);background: rgba(255,255,255,0.90);font-weight:1000;color:#0b0b0f;box-shadow:0 10px 18px rgba(40,10,70,0.06);}
.chip.on{background: linear-gradient(135deg, rgba(255,79,216,0.18), rgba(185,130,255,0.16));border-color: rgba(255,79,216,0.26);}
...
.ghostBtn{height:44px;padding:0 12px;border-radius:16px;border:1px solid rgba(18,6,26,0.12);background: rgba(255,255,255,0.92);font-weight:1000;color:#0b0b0f;box-shadow:0 12px 24px rgba(40,10,70,0.06);}
.grantMsg{margin-top:10px;font-size:13px;font-weight:1000;color:#0b0b0f;}

.panelActions{margin-top:10px;display:flex;gap:8px;}
.pAct{flex:1;height:40px;border-radius:14px;border:1px solid rgba(18,6,26,0.14);font-weight:1000;color:#0b0b0f;background: rgba(255,255,255,0.92);box-shadow:0 12px 24px rgba(40,10,70,0.08);}
...
.footBtn{width:100%;height:44px;border-radius:16px;border:1px solid rgba(18,6,26,0.14);background: linear-gradient(135deg, rgba(255,79,216,0.16), rgba(185,130,255,0.14));font-weight:1000;color:#0b0b0f;box-shadow:0 14px 30px rgba(40,10,70,0.10);}
`;
