...
function safeDealFromNotes(notes: any | null | undefined): DealData {
  const d = notes?.deal || {};
  return {
    ...
    manageNote: typeof d?.manageNote === 'string' ? d.manageNote : '',
  };
}
...
export default function CustomerDetailPage() {
    ...
    const run = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('customers')
          .select('id,user_id,name,phone,stage,grade,memo,notes_json,created_at')
          .eq('id', id)
          .single();
        ...
        // ✅ notes_json.deal 로드 (신버전)
        setDeal(safeDealFromNotes(loaded?.notes_json));
      } catch (e) {
        console.error(e);
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      // ✅ 기존 notes_json 유지 + deal만 merge
      const prevNotes = row.notes_json && typeof row.notes_json === 'object' ? row.notes_json : {};
      const nextNotes: any = { ...prevNotes };

      if (isDealMeaningful(deal)) {
        nextNotes.deal = {
          product: (deal.product || '').trim(),
          issue: (deal.issue || '').trim(),
          amount: typeof deal.amount === 'number' ? deal.amount : null,
          manageNote: (deal.manageNote || '').trim(),
        };
      } else {
        if (nextNotes?.deal) delete nextNotes.deal;
      }
      ...
      // ✅ 화면 state도 최신 반영
      setRow((p) => (p ? { ...p, notes_json: nextNotes } : p));
        ...
        {loading ? (
          ...
          <>
              ...
              <div className="grid">
                <label className="field">
                  ...
                  <input
                    className="input"
                    value={row?.name ?? ''}
                    onChange={(e) => setRow((p) => (p ? { ...p, name: (e.target as HTMLInputElement).value } : p))}
                  />
                </label>
                ...
                <label className="field">
                  ...
                  <input
                    ...
                    value={row?.phone ?? ''}
                    onChange={(e) =>
                      setRow((p) => (p ? { ...p, phone: (e.target as HTMLInputElement).value } : p))
                    }
                  />
                </label>
                ...
                <label className="field">
                  ...
                  <select
                    ...
                    value={row?.stage ?? '신규'}
                    onChange={(e) =>
                      setRow((p) => (p ? { ...p, stage: (e.target as HTMLSelectElement).value } : p))
                    }
                  >
                ...
                <label className="field">
                  ...
                  <select
                    ...
                    value={row?.grade ?? 'A'}
                    onChange={(e) =>
                      setRow((p) => (p ? { ...p, grade: (e.target as HTMLSelectElement).value } : p))
                    }
                  >
                ...
                <label className="field full">
                  ...
                  <textarea
                    ...
                    value={row?.memo ?? ''}
                    onChange={(e) =>
                      setRow((p) => (p ? { ...p, memo: (e.target as HTMLTextAreaElement).value } : p))
                    }
            ...
            <section className="card dealWrap">
              ...
              />
              <div className="dealHint">※ 상품/금액/이슈/상담내역은 저장 시 함께 저장됩니다.</div>
            </section>
          </>
        )}
